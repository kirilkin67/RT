...
int		ft_illumination_point(t_light *l, t_sphere *s, t_vector *v)
{
	t_vector	light;
	t_vector	normal;
	t_vector	median;
	...
	shade = ft_vector_scalar(&normal, &light) / ft_vector_modul(&normal) / ft_vector_modul(&light);
	if (shade < 0)
		shade = 0;
	shine = ft_vector_scalar(&normal, &median) / ft_vector_modul(&normal) / ft_vector_modul(&median);
	if (shine < 0)
		shine = 0;
...
int		ft_intersect_ray_sphere(t_camera *r, t_sphere *s, t_light *l)
{
	...
	len_s = ft_vector_projection_on_ray(&s->pos, &s->pos);
	len_dist = pow(s->radius, 2) - (pow(len_s, 2) - pow(len_dir, 2));
	if (len_dist < 0)
		return (-1);
	else
	{
		interset = ft_vector_intersect(&r->dir, len_dir, len_dist);
		point_color = ft_illumination_point(l, s, &interset);
	}
	return (point_color);
}
